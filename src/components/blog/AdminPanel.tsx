...
const SESSION_KEY = "ks-blog-admin-key";
...
export default function AdminPanel() {
  ...
  function handleUnlock(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (keyInput === ADMIN_KEY) {
      window.sessionStorage.setItem(SESSION_KEY, keyInput);
      setUnlocked(true);
      setGateError(null);
    } else {
  ...
  async function act(post: Blog, status: "published" | "rejected") {
    if (!keyInputTorSaved()) return;
    const adminKey = getAdminKey();

    setRows((prev) => ({ ...prev, [post.id]: { saving: true, error: null } }));
    try {
      await moderateBlog({
        id: post.id,
        status,
  ...
  function keyInputTorSaved(): boolean {
    if (!ADMIN_KEY) {
    ...
    return (
      <div className="mx-auto max-w-md border-2 border-ink bg-cream p-8 shadow-[8px_8px_0px_0px_var(--color-ink)]">
        ...
        <p className="mt-2 text-sm font-medium text-ink/60">
          Enter the shared admin key to open the moderation queue.
        </p>
        ...
        <form onSubmit={handleUnlock} className="mt-6 space-y-4">
          ...
          <div>
            <label htmlFor="admin-key" className="block text-xs font-bold uppercase tracking-widest">
              Admin key
            </label>
            <input
              type="password"
      ...
      {!loading && posts.length === 0 && !queueError && (
        <div className="flex flex-col items-center justify-center gap-3 border-2 border-dashed border-ink py-16 text-center">
          ...
          <p className="max-w-sm text-sm font-medium text-ink/60">
            No posts waiting for review. New submissions appear here as they
            come in.
          </p>
        </div>
      )}
      ...
      {posts.map((post, i) => {
        ...
        return (
          ...
          >
            ...
            {row?.error && (
              <div className="mt-4 border-2 border-ink bg-[#fecaca] px-4 py-3 text-sm font-bold">
                {row.error}
              </div>
            )}
